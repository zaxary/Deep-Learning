...
class ReplayMemory(object):
    ...
    def push(self, history, action, reward, done):
        self.memory.append((history, action if isinstance(action, int) else action.cpu(), reward, done))
...
class ReplayMemoryLSTM(ReplayMemory):
    ...
    def sample_mini_batch(self, frame):
        ...
        for i in idx_sample:
            ...
            sample = np.array(sample)
            mini_batch.append((np.stack(sample[:, 0], axis=0), sample[lstm_seq_length - 1, 1],
                               sample[lstm_seq_length - 1, 2], sample[lstm_seq_length - 1, 3]))
